...
class GhostOnSkates(Problem):
    ...
    @staticmethod
    def check_valid(state, walls, board):
        if  state[0] < 0 or state[0] >=board or \
            state[1] < 0 or state[1] >= board or \
            state in walls:
                return False
        return True
    ...
    def h(self, node):
        #return abs((self.n-1)-node.state[0]) + abs((self.n-1)-node.state[1])
        # p = list(node.state)
        # q = list((self.n-1, self.n-1))
        # return math.dist(p, q)
        return math.sqrt((self.n - 1 - node.state[0]) ** 2 + (self.n - 1 - node.state[1]) ** 2)
