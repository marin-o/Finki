...
if __name__ == '__main__':
    ...
    train_naive = zero_class[:int(x1 * len(zero_class))] + one_class[:int(x1 * len(one_class))] + two_class[:int(x1 * len(two_class))]
    train_tree = zero_class[int(x1 * len(zero_class)):int(x2 * len(zero_class))] + one_class[int(x1 * len(one_class)):int(x2*len(one_class))] + two_class[int(x1 * len(two_class)):int(x2*len(two_class))]
    train_forest = zero_class[:int(x2 * len(zero_class))] + one_class[:int(x2 * len(one_class))] + two_class[:int(x2 * len(two_class))]

    test_set = zero_class[int(x2 * len(zero_class)):] + one_class[int(x2 * len(one_class)):] + two_class[int(x2 * len(two_class)):]
    test_x = [row[:-1] for row in test_set]
    test_y = [row[-1] for row in test_set]

    train_x_naive = [row[:-1] for row in train_naive]
    train_y_naive = [row[-1] for row in train_naive]

    train_x_tree = [row[:-1] for row in train_tree]
    train_y_tree = [row[-1] for row in train_tree]

    train_x_forest = [row[:-1] for row in train_forest]
    train_y_forest = [row[-1] for row in train_forest]

    naive.fit(train_x_naive, train_y_naive)
    tree.fit(train_x_tree, train_y_tree)
    forest.fit(train_x_forest, train_y_forest)

    accuracy = 0
    for true, pred_naive, pred_tree, pred_forest in zip (test_y, naive.predict(test_x), tree.predict(test_x), forest.predict(test_x)):
        accuracy += 1 if sum([true == pred_naive, true == pred_tree, true == pred_forest]) >= 2 else 0
