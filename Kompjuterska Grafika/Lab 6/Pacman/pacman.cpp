...
int main() {
  ...
  Shader ourShader(shader_location + used_shaders + std::string(".vert"),
    ...
    const float pacmanAngle = glm::pi<float>() / 1.15f; // Angle for the pacman mouth
    ...
    for (int i = 0; i <= stacks; ++i) {
        ...
        for (int j = 0; j <= sectors; ++j) {
            ...
            // Check if the vertex is outside the Pacman's mouth angle
            if (sectorAngle < (glm::pi<float>() - pacmanAngle) || sectorAngle > (glm::pi<float>() + pacmanAngle)) {
                // Set the vertex position to the center of the sphere
                x = 0.0f;
                y = 0.0f;
                z = 0.0f;
            }
