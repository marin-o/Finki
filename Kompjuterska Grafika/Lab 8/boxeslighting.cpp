...
// lighting
static glm::vec3 lightPos(4.0f, 1.0f, -4.0f);
...
int main() {
    ...
    unsigned int diffuseMap =
            loadTexture((texture_location + "container2_blue.png").c_str());
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        // render the cubes
        glBindVertexArray(cubeVAO);
        GLint loc = glGetUniformLocation(lightingShader.ID, "time");
        for (unsigned int i = 0; i < 10; i++) {
            ...
            float angle;
            angle = ((i+1.5f)*glfwGetTime()) * 10.0f;
