...
void processInput(GLFWwindow *window, GLuint shaderProgram, float &x, float &y);
...
//movement speed
const float movementSpeed = 0.00025f;
//bounds
...
int main()
{
    ...
    for (auto i=0; i<9; ++i) {
        vertices.push_back(glm::cos(angle) * 0.25f);
        vertices.push_back(glm::sin(angle) * 0.25f);
        vertices.push_back(0.0f);
        angle += PI_OVER_4;
    }
    ...
    while (!glfwWindowShouldClose(window))
    {
        // input
        // -----
        processInput(window, shaderProgram, x, y);
...
void processInput(GLFWwindow *window, GLuint shaderProgram, float &x, float &y)
{
    ...
    // Update the vertex buffer data with the new positions
    glBindBuffer(GL_ARRAY_BUFFER, VBO);
    glBufferData(GL_ARRAY_BUFFER, vertices.size() * sizeof(vertices), &vertices[0], GL_STATIC_DRAW);
    glBindBuffer(GL_ARRAY_BUFFER, 0);
}
