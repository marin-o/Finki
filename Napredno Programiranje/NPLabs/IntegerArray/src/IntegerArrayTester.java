...
final class IntegerArray{
    ...
    public IntegerArray concat(IntegerArray arr){
        int[] newArr = new int[arr.length()+a.length];
        for(int i=0;i<a.length + arr.length();i++){
            if(i>=a.length)
                newArr[i] = arr.getElementAt(i-a.length);
            else newArr[i]=a[i];
        }
