...
public class Speluvanje {
    public static void main(String[] args) throws IOException {
        ...
        for ( int i=0; i < N; i++ ) {
            Zbor z=new Zbor(br.readLine());
            tabela.insert(z, z.zbor);
        }
        boolean flag = true;
        String[] tekst=br.readLine().trim().replaceAll("[.,?!]", "").split("\\s+");
        for ( String zbor: tekst ) {
            if ( tabela.search(new Zbor(zbor.toLowerCase())) < 0 && zbor.length()>0) {
