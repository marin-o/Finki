...
class Football(Problem):
    def __init__(self, o, ill, initial, goal=None):
    ...
    @staticmethod
    def check_valid(state, opponents, illegal):
        man_pos = state[0]
        ball_pos = state[1]
        return 0 <= man_pos[0] < 8 and \
            ...
            ball_pos not in illegal

    def successor(self, state):  # (man_pos,ball_pos) = ((mx,my),(bx,by))
        successors = dict()
        mx, my, bx, by = state[0][0], state[0][1], state[1][0], state[1][1]
        # pomesti coveche gore
        new_man = (mx, my+1)
        if new_man[1] < 6 and new_man not in self.opponents:
            # turni topka gore
            if new_man == (bx, by):
                new_ball = (bx, by+1)
                if new_ball[1] < 6 and new_ball not in self.illegal:
                    successors['Turni topka gore'] = (new_man, new_ball)
            else:
                successors['Pomesti coveche gore'] = (new_man, state[1])
        # pomesti coveche dolu
        new_man = (mx, my-1)
        if new_man[1] >= 0 and new_man not in self.opponents:
            # turni topka dolu
            if new_man == (bx, by):
                new_ball = (bx, by-1)
                if new_ball[1] >=0 and new_ball not in self.illegal:
                    successors['Turni topka dolu'] = (new_man, new_ball)
            else:
                successors['Pomesti coveche dolu'] = (new_man, state[1])
        # pomesti coveche desno
        new_man = (mx+1, my)
        if new_man[0] < 8 and new_man not in self.opponents:
            #turni dopka desno
            if new_man == (bx, by):
                new_ball = (bx+1, by)
                if new_ball[0] < 8 and new_ball not in self.illegal:
                    successors['Turni topka desno'] = (new_man, new_ball)
            else:
                successors['Pomesti coveche desno'] = (new_man, state[1])
        # pomesti coveche levo
        new_man = (mx-1, my)
        if new_man[0] >= 0 and new_man not in self.opponents:
            #turni dopka desno
            if new_man == (bx, by):
                new_ball = (bx-1, by)
                if new_ball[0] >= 0 and new_ball not in self.illegal:
                    successors['Turni topka levo'] = (new_man, new_ball)
            else:
                successors['Pomesti coveche levo'] = (new_man, state[1])
        # pomesti coveche gore-desno
        new_man = (mx+1, my+1)
        if new_man[0] < 8 and new_man[1] < 6 and new_man not in self.opponents:
            #turni topka gore-desno
            if new_man == (bx, by):
                new_ball = (bx+1, by+1)
                if new_ball[0] < 8 and new_ball[1] < 6 and new_ball not in self.illegal:
                    successors['Turni topka gore-desno'] = (new_man, new_ball)
            else:
                successors['Pomesti coveche gore-desno'] = (new_man, state[1])
        # pomesti coveche dolu-desno
        new_man = (mx+1, my-1)
        if new_man[0] < 8 and new_man[1] >= 0 and new_man not in self.opponents:
            #turni topka gore-desno
            if new_man == (bx, by):
                new_ball = (bx+1, by-1)
                if new_ball[0] < 8 and new_ball[1] >= 0 and new_ball not in self.illegal:
                    successors['Turni topka dolu-desno'] = (new_man, new_ball)
            else:
                successors['Pomesti coveche dolu-desno'] = (new_man, state[1])
        return successors
...
if __name__ == '__main__':
    ...
    opponents = [(3, 3), (5, 4)]
    illegal_positions_ball = list()
    for i in range(opponents[0][0]-1, opponents[0][0]+2):
        for j in range(opponents[0][1]-1, opponents[0][1]+2):
            if (i, j) not in illegal_positions_ball:
                illegal_positions_ball.append((i, j))

    for i in range(opponents[1][0]-1, opponents[1][0]+2):
        for j in range(opponents[1][1]-1, opponents[1][1]+2):
            if (i,j) not in illegal_positions_ball:
                illegal_positions_ball.append((i, j))
