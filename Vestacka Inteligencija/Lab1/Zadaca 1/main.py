...
class Snake(Problem):
    ...
    def successor(self, state):  # ('dir',snake[(),(),...],greenApples)
        # game = state   # state[0]=dir; state[1] = snake; state[2]=greenApples=[(),(),...]
        successors = dict()
        direction = state[0]
        zmija = list(state[1])
        greenApples = list(state[2])

        # zmija = game[1]  # zmija[0]=glava; zmija[1,2,...]=telo
        # glava = zmija[0]
        if state[0] == 's':  # facing down
            # prodolzi pravo
            if zmija[0][1] - 1 >= 0 and (zmija[0][0], zmija[0][1] - 1) not in self.red_apples and (zmija[0][0], zmija[0][1] - 1) not in zmija[:-1]:
                if (zmija[0][0], zmija[0][1] - 1) in greenApples:
                    zmija.insert(0, (zmija[0][0], zmija[0][1] - 1))
                    greenApples.remove((zmija[0][0], zmija[0][1]))
                else:
                    zmija.insert(0, (zmija[0][0], zmija[0][1] - 1))
                    del zmija[-1]
                direction = direction
                successors['ProdolzhiPravo'] = (direction, tuple(zmija), tuple(greenApples))
            # svrti desno
            if zmija[0][0] - 1 >= 0 and (zmija[0][0] - 1, zmija[0][1]) not in self.red_apples and (zmija[0][0] - 1, zmija[0][1]) not in zmija[:-1]:
                if (zmija[0][0] - 1, zmija[0][1]) in greenApples:
                    zmija.insert(0, (zmija[0][0] - 1, zmija[0][1]))
                    greenApples.remove((zmija[0][0], zmija[0][1]))
                else:
                    zmija.insert(0, (zmija[0][0] - 1, zmija[0][1]))
                    del zmija[-1]
                direction = 'w'
                successors['SvrtiDesno'] = (direction, tuple(zmija), tuple(greenApples))
            # svrti levo
            if zmija[0][0] + 1 < 10 and (zmija[0][0] + 1, zmija[0][1]) not in self.red_apples and (zmija[0][0] + 1, zmija[0][1]) not in zmija[:-1]:
                if (zmija[0][0] + 1, zmija[0][1]) in greenApples:
                    zmija.insert(0, (zmija[0][0] + 1, zmija[0][1]))
                    greenApples.remove((zmija[0][0], zmija[0][1]))
                else:
                    ...
                    del zmija[-1]
                direction = 'e'
                successors['SvrtiLevo'] = (direction, tuple(zmija), tuple(greenApples))
        elif state[0] == 'n':  # facing up
            # prodolzi pravo
            if zmija[0][1] + 1 < 10 and (zmija[0][0], zmija[0][1] + 1) not in self.red_apples and (zmija[0][0], zmija[0][1] + 1) not in zmija[:-1]:
                if (zmija[0][0], zmija[0][1] + 1) in greenApples:
                    zmija.insert(0, (zmija[0][0], zmija[0][1] + 1))
                    greenApples.remove((zmija[0][0], zmija[0][1]))
                else:
...
if __name__ == '__main__':
    ...
    green_apples = list()
    red_apples = list()
    greencounter = int(input())
    for i in range(greencounter):
        inp = input().split(",")
        green_apples.append(tuple([int(inp[0]), int(inp[1])]))
        # green_apples.append(tuple([int(inp[0]), int(inp[1])-1]))
    redcounter = int(input())
    for i in range(redcounter):
        inp = input().split(",")
        red_apples.append(tuple([int(inp[0]), int(inp[1])]))

    snake = tuple([(0, 7), (0, 8), (0, 9)])



    init = ('s', snake, tuple(green_apples))
    prob = Snake(tuple(red_apples), init)
    if not green_apples:
        print(green_apples)
    else:
        print(breadth_first_graph_search(prob).solution())